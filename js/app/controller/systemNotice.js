define([
	...
	function ajaxUpdata(sta,lim){
		...
		Ajax.get("804040",{"channelType": "4","pushType": "41","start":sta,"limit": lim})
		.then(function(res){
			if(res.success){
				...
				var img = __inline("../images/icon-news.png");
...
//							console.log("已加载消息条数"+num,"总消息条数"+sum)
						...
						s += "<div class='pr10 flex-s newp'><img src='"+img+"' class='newp'/></div>";
			    		s += "<div  style='padding-bottom: 25px;'><div>";
			        	s += "<div class='fs14'>"+smsTitle+"</div>";
			        	s += "<div class='fs12'  style='color: #999999'>"+pushedDatetime+"</div>";
			    		s += "</div><div class=' bg_f5 ptb15 plr10 fs12 mt10' style='color: #999;border-radius:4px;line-height:20px;'>"+smsContent+"</div></div></div>";
