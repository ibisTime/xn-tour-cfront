define("app/views/common/SideMenu1", [
    'dojo/_base/declare',
    'dijit/_WidgetBase',
    ...
    'dojo/dom-style',
    'dojo/_base/config',
    'dojo/text!./templates/SideMenu1.html'
    ...
    return declare([_WidgetBase, _TemplatedMixin, ViewMixin], {
        ...
        _setActiveAttr: function(value) {
            ...
            if (subIndex) {
                var fEl = query('>li', this.subMenuNode)[index - 1];
                var next = fEl.nextElementSibling || fEl.nextSibling;
                var cEl = query('>li', next)[subIndex - 1];
                this.expandItem(query('>li', this.subMenuNode)[index - 1]);
                domClass.add(cEl, 'active');
            } else {
                domClass.add(query('>li', this.subMenuNode)[index - 1], 'active');
            }
        },

        render: function() {
            var me = this;
            on(this.subMenuNode, 'li:click', function(e) {
        ...
        collapseAll: function(item) {
            var me = this;
            var items = query('ul', this.subMenuNode).prev();
