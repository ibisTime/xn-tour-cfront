...
define("app/common/SVG", [
    ...
    return {
        circleProgress: function(el, value, width, rad) {
            ...
            paper.circle(width / 2,width / 2,width / 2 - 5 - 5).attr({ stroke: 'none', fill: '#193340' });
            ...
            var z = paper.path().attr({arc: [value, colors[index], rad - 10], 'stroke-width': 5 });
            z.mouseover(function(){
                this.animate({ 'stroke-width': 10, opacity: .75 }, 1000, 'elastic');
                if(Raphael.type != 'VML') //solves IE problem
                    this.toFront();
            }).mouseout(function(){
                this.stop().animate({ 'stroke-width': 5, opacity: 1 }, 1000, 'elastic');
            });
        }
    };
});
