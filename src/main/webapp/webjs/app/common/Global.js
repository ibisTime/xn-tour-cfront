...
define("app/common/Global", [
	...
	return {
		 ...
		 * @param value number
		 * @param places number -- numbers of digital after dot, places value 0 - 3
		 * @return
		 */
        formatAmount: function(value, places, unit, carry) {
			...
			value = '' + value;
			var len = Math.min(3 - places, value.length);
			value = value.slice(0, value.length - len) + '00000000'.slice(0, len);
			value = value / 10;
		...
		parseAmount: function(value) {
			return this.roundAmount(parseFloat(value) * 1000, 0);
		},


        switchAmount: function(value, unit, carry) {
            // unit: 1为元，0为分， carry：1为舍去，0为四舍五入
            var tail = ('00' + value).split('.')[1] || '0';
            var symbol = ((value + '')[0] == '-') ? '-' : '';
            value = ('00' + Math.abs(value)).split('.')[0];

            if (unit == 1) { // 元
                if (carry == 1) {
                    return parseInt(symbol + value.slice(0, value.length - 2) + '00');
                } else {
                    var prefixStr = value.slice(0, value.length - 2),
                        tenPos = value.slice(value.length - 2, value.length - 1);
                    if (tenPos < (5 + '')) {
                        return parseInt(symbol + prefixStr + '00');
                    } else {
                        return parseInt(symbol + (parseInt(prefixStr) + 1) + '00');
                    }
                }
            } else {
                if (carry == 1) {
		...
		sirOrLady: function(params) {
			...
			} else if (params.idNo && params.realName) {
				var userRealName=params.realName.replace(/\s+/g,"");
				var first_name = userRealName.length == 4 ? userRealName.slice(0,2) :
					userRealName.slice(0,1);
				if (params.idNo.length == 15) {
					res = first_name + (parseInt(params.idNo.slice(14, 15)) % 2 ? '先生' : '女士');
				} else {
					res = first_name + (parseInt(params.idNo.slice(16, 17)) % 2 ? '先生' : '女士');
				}
			} else {
				res = params.mobile || params.email;
			}
			return res;
		},
        ...
        getUrlParam: function(param) {
            var paramIndex = location.href.indexOf('?'), paramsObj = {};
            if (paramIndex > 0) {
        ...
        getUrlParam2: function(param) {
        	var pattern = new RegExp(param,'gi');

        	if(pattern.test(location.href)){
        		return location.href.replace(param,'@').split('@')[1].slice(2);
        	}
        },
