    ...
    return {
        ...
        getBanks: function () {
            ...
            return banks || Ajax.get(Global.baseUrl + '/bank', {
                'pay_company_no': payCompanyNo//1:通联 2：易宝  3：云融
            }).then(function (response) {
                if (response.success) {
                    banks = response.data;
                }
        ...
        getAgreement: function (params, reload) {
            if (reload) {
                protocol = null;
            }
            return protocol || Ajax.get(Global.baseUrl + '/product/agreement',params).then(function (response) {
                if (response.success) {
                	protocol = response.data.agreement;
                }
                return protocol;
            });
        },
        ...
        getSysParams: function (params, reload) {
        if (reload) {
            cache['sysparam' + params.param_code] = null;
        }
        return cache['sysparam' + params.param_code] || Ajax.get(Global.baseUrl + '/sysparams', params).then(function (response) {
                if (response.success) {
                    cache['sysparam' + params.param_code] = response.data;
                }
                return cache['sysparam' + params.param_code];
            });
        }
    }
});