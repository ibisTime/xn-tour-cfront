define("app/jquery/CountDown", [
], function() {
    (function(exports) {
        var zDate = {parse: function(str) {
            if (/^\d{10}$/.test(str)) {
            ...
            var o = {"M+": date.getMonth() + 1,"d+": date.getDate(),"h+": date.getHours(),"m+": date.getMinutes(),"s+": date.getSeconds(),"q+": Math.floor((date.getMonth() + 3) / 3),"S": date.getMilliseconds(),"w": "日一二三四五六".charAt(date.getDay())};
            ...
            var start = startTime ? this.parse(startTime) : null, end = endTime ? this.parse(endTime) : null, now = nowTime ? this.parse(nowTime) : new Date(), flag = false;
            if (start && end && now) {
                start = start.getTime();
                now = now.getTime();
                end = end.getTime();
            ...
            var dayBox = params.dayBox, hourBox = params.hourBox, minuteBox = params.minuteBox, secondBox = params.secondBox, milliSecondBox = params.milliSecondBox, nowTime = params.nowTime, startTime = params.startTime, endTime = params.endTime, nowTimeObj = nowTime && this.parse(nowTime), endTimeObj = endTime && this.parse(endTime), interval = params.interval || 1000, ms, hashTime = 0, timeObj, self = this;
            if (!nowTimeObj) {
                nowTimeObj = new Date();
            }
            if (!endTimeObj) {
            ...
            return {stopCountDown: function() {
                clearTimeout(timeCounter);
            }}
        }};
        exports.Date = {parse: function(text) {
