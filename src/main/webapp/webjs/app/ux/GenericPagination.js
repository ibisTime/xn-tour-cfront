define("app/ux/GenericPagination", [
	"dojo/_base/kernel",
	"dojo/_base/declare",
	"dojo/_base/array",
	"dojo/_base/connect",
...
], function(kernel, declare, array, connect, lang, html, event, query, on,
...
var _Paginator = declare("dojox.grid.enhanced.plugins._Paginator", [_Widget, _TemplatedMixin], {
	...
	_placeSelf: function(){
		...
		this.placeAt(isTop ? g.viewsHeaderNode : g.messagesNode, isTop ? "before" : "after");
...
//		createWardBtn("lastPage", nls[9], "last");
		...
		if(this._currentPage){
			html.create('div', {innerHTML: this._currentPage}, this.pageStepperNode, 'last');
		}else {
			html.create('div', {innerHTML: 0}, this.pageStepperNode, 'last');
		}
...
//		}, this);
		...
		var curPageSpan = this.pageStepperNode.childNodes[1];
		var nextBtn = this.pageStepperNode.childNodes[2];
		html.removeClass(preBtn, 'genericButtonDisabled');
		html.removeClass(nextBtn, 'genericButtonDisabled');
		html.addClass(curPageSpan, 'genericNum');
		
		var pageHTML = "";
		if(pageCount > 9){
			var minShowPage = Math.min(pageCount-8, Math.max(1,curPage-4));
			var maxShowPage = Math.min(pageCount, minShowPage+8);
			if(minShowPage!=1){
				pageHTML += "...&nbsp;";
			}
		...
		curPageSpan.innerHTML = pageHTML;
		
		var thiz = this;
		on(query(".genericNum a"), "click", function(evt){
			var _toPage = parseInt(evt.target.innerHTML);
			thiz.plugin.currentPage(_toPage);
		});
