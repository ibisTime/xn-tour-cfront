define("app/ux/TimeShareChart", [
    'app/jquery/$',
    'app/jquery/HighStock'
], function() {
    var todayTime = new Date();
    var timeShareRefreshFunction = null;
    var timeShareUpdateInternal = 5000; // 30 seconds
...
// 数组数据比较大小
    ...
    function refreshTitle(self, stockName, obj) {
        if (!obj) {
            self.setTitle({
                text: '<span style="font-weight: bold;font-size: 18px; color: #333;">'+stockName+'</span><span style="margin-left: 10px;color: #DD2200">数据异常</span>'
            });
            return;
        }
        var lastTrend = obj['trendList'][obj['trendList'].length - 1];
        var diff = lastTrend.newPrice - obj.partRealTimeData.prevClosePrice;
        if (!parseFloat(lastTrend.newPrice)) {
            self.setTitle({
                text: '<span style="font-weight: bold;font-size: 18px; color: #333;">'+stockName+'</span><span style="margin-left: 10px; ">--</span><span>--%(--)</span>'
            });
        } else {
            self.setTitle({
                text: '<span style="font-weight: bold;font-size: 18px; color: #333;">'+stockName+'</span><span style="margin-left: 10px; color: '+(diff > 0 ? '#DD2200' : '#33AA11') + '">' + parseFloat(lastTrend.newPrice).toFixed(2) + '</span><i style="color: '+(diff > 0 ? '#DD2200' : '#33AA11') + '" class="fa '+(diff > 0 ? 'fa-arrow-up' : 'fa-arrow-down')+'"></i><span style="margin-left: 10px;color: '+(diff > 0 ? '#DD2200' : '#33AA11') + '">'+diff.toFixed(2)+'('+(diff / obj.partRealTimeData.prevClosePrice * 100).toFixed(2)+'%)</span>'
            });
        }
    ...
    // 画分时空图
    var drawEmptyChart = function(renderDivId, stockName) {
        if (!document.getElementById(renderDivId)) {
            return;
        }
        var chartHeight = 192;
        var priceChartHeight = document.getElementById(renderDivId).offsetHeight - 72;
        ...
        var chart = new Highcharts.StockChart(timeShareOption);
        refreshTitle(chart, stockName);
    };

// 画分时图
    var drawTimeShareChart = function(renderDivId, stockCode, url, stockName) {
        if (!document.getElementById(renderDivId)) {
            return;
        }
        //置空
        ...
        if (typeof (url) == "undefined" || url == null) {
            if (typeof (chart) != "undefined" && chart != null)
                ...
                drawEmptyChart(renderDivId, stockName);
            return;
        }
        var priceChartHeight = document.getElementById(renderDivId).offsetHeight - 72;
        $.ajax({
            ...
            success : function(res) {
                    ...
                    if (timeSharePriceData.length >= resp['trendList'].length) {
                        ...
                        for ( var i = 0; i < newData.length; i++) {
                            if (parseFloat(newData[i].total) < 0.0){
                                ...
                                if(/*timeShareLastPrice == null*/true){
                                    ...
                                    $.ajax({
                                        ...
                                        success : function(res) {
                                            if (res.error != null) {
                                                ...
                                                var resp = res['data'];
                                                refreshTitle(that, stockName, resp);
                                                if (resp['trendList'] != null&& typeof (resp['trendList']) != undefined&& resp['trendList'].length > 0) {
                                                    if (timeSharePriceData.length >= resp['trendList'].length) {
                                                        return;
                                                    } else {
                ...
                var chart = new Highcharts.StockChart(timeShareOption);
                refreshTitle(chart, stockName, resp);
